...
namespace WebApplication1.Controllers
{
    public class LicenseController : Controller
    {
        ...
        public LicenseController(IConfiguration configuration, IWebHostEnvironment webHostEnvironment)
        {
        ...
        public IActionResult Index()
        {
            try
            {
                var viewModel = new LicenseIndexViewModel
                {
                    ...
                    EmployeeLicenses = _db.GetEmployeeLicenses()
                };

                foreach (var license in viewModel.ControllerLicenses.Concat(viewModel.EmployeeLicenses))
                {
                    ...
                    page.Content().Table(table =>
                    {
                        ...
                        else // Employee
                        {
                            table.ColumnsDefinition(columns =>
                            {
                                ...
                                columns.RelativeColumn(2.5f); // Permission Type
                                columns.RelativeColumn(1.5f); // Issue Date
                                columns.RelativeColumn(1.5f); // Expiry Date
                            });
                            ...
                            table.Header(header =>
                            {
                                ...
                                header.Cell().Element(HeaderStyle).Text("Department");
                                header.Cell().Element(HeaderStyle).Text("Permission Type");
                                header.Cell().Element(HeaderStyle).Text("Issue Date");
                                header.Cell().Element(HeaderStyle).Text("Expiry Date");
                            });
            ...
            using (var package = new ExcelPackage())
            {
                ...
                foreach (var license in licenses)
                {
                    ...
                    else // Employee
                    {
                        ...
                        worksheet.Cells[row, 5].Value = license.IssueDate;
                        worksheet.Cells[row, 6].Value = license.ExpiryDate;
                        worksheet.Cells[row, 5, row, 6].Style.Numberformat.Format = "yyyy-mm-dd";
                    }
                    row++;
                }
