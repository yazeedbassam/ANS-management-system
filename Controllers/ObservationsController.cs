    ...
    public class ObservationsController : Controller // Make sure the name ends with 's'
    {
        ...
        public IActionResult Index()
        {
            try
            {
                var viewModel = new ObservationIndexViewModel
                {
                    ...
                    EmployeeObservations = _db.GetEmployeeObservations()
                };
        ...
        public IActionResult ExportToPDF(string personType, string observationNo, string personName, string flightNo, string travelCountry)
        {
            ...
            if (personType == "Controller")
            {
                ...
                if (!string.IsNullOrEmpty(personName)) observations = observations.Where(o => o.ControllerName != null && o.ControllerName.Contains(personName, StringComparison.OrdinalIgnoreCase)).ToList();
            }
            else
            {
                observations = _db.GetEmployeeObservations();
                reportTitle = "Users Observations Report";
                if (!string.IsNullOrEmpty(personName)) observations = observations.Where(o => o.EmployeeName != null && o.EmployeeName.Contains(personName, StringComparison.OrdinalIgnoreCase)).ToList();
            }

            if (!string.IsNullOrEmpty(observationNo)) observations = observations.Where(o => o.ObservationNo.ToString() == observationNo).ToList();
            if (!string.IsNullOrEmpty(flightNo)) observations = observations.Where(o => o.FlightNo != null && o.FlightNo.Contains(flightNo, StringComparison.OrdinalIgnoreCase)).ToList();
            if (!string.IsNullOrEmpty(travelCountry)) observations = observations.Where(o => o.TravelCountry != null && o.TravelCountry.Contains(travelCountry, StringComparison.OrdinalIgnoreCase)).ToList();
        ...
        public IActionResult ExportToExcel(string personType, string observationNo, string personName, string flightNo, string travelCountry)
        {
            ...
            // 1. جلب البيانات وفلترتها بناءً على نوع التاب والفلاتر
            if (personType == "Controller")
            {
                observations = _db.GetControllerObservations();
                reportTitle = "Controller Observations Report";
