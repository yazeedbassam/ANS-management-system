...
WHERE
    l.expirydate <= DATEADD(day, 90, GETDATE());
        ...
        public byte[] GenerateWeeklyReportPDF(DataTable soonExpiringTable, int expiredCount, int soonExpiringCount) // <== تم تغيير الوصول إلى public
        {
                    ...
                    page.Content().PaddingTop(15).Column(mainCol =>
                    {
                        ...
                        mainCol.Item().Table(table =>
                        {
                            ...
                            table.Header(header =>
                            {
                                ...
                                header.Cell().Element(CellStyleHeader).Text("الاسم");
                                header.Cell().Element(CellStyleHeader).Text("نوع الرخصة");
                                header.Cell().Element(CellStyleHeader).Text("تاريخ الانتهاء");
                                header.Cell().Element(CellStyleHeader).Text("Mobile");
                            ...
                            foreach (DataRow r in soonExpiringTable.Rows)
                            {
                                ...
                                table.Cell().Element(CellStyleBody).Text(r["fullname"].ToString() ?? string.Empty);
                                table.Cell().Element(CellStyleBody).Text(r["licensetype"].ToString() ?? string.Empty);
                                table.Cell().Element(CellStyleBody).Text(Convert.ToDateTime(r["expirydate"]).ToString("yyyy-MM-dd"));
                                table.Cell().Element(CellStyleBody).Text(r["phone_number"].ToString() ?? string.Empty);
                                table.Cell().Element(CellStyleBody).Text(r["email"].ToString() ?? string.Empty);
                            }
                        });
                    });
            ...
            // الدوال المحلية لتنسيق الجدول في PDF (QuestPDF)
            // هذه الدوال يجب أن تكون داخل دالة GenerateWeeklyReportPDF
            IContainer CellStyleHeader(IContainer container) =>
                container.Background(QuestPDF.Helpers.Colors.Blue.Medium)
                         .PaddingVertical(6).PaddingHorizontal(2)
                         .DefaultTextStyle(x => x.FontColor(QuestPDF.Helpers.Colors.White).FontSize(13).SemiBold());

            IContainer CellStyleBody(IContainer container) =>
                container.BorderBottom(1).BorderColor(QuestPDF.Helpers.Colors.Grey.Lighten2)
                         .PaddingVertical(5).PaddingHorizontal(2)
                         .DefaultTextStyle(x => x.FontSize(12).FontColor(QuestPDF.Helpers.Colors.Grey.Darken3));
        }
