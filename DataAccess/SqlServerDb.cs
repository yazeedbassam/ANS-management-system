...
namespace WebApplication1.DataAccess // Assuming this namespace remains the same
{
    ...
    public class SqlServerDb
    {
        ...
        SELECT
          ...
          c.transfer_date, c.emergency_contact,c.LicenseNumber, a.airportname, a.icao_code, co.CountryName
        FROM controllers c
        LEFT JOIN 
...
AND (
    ...
    OR LOWER(s.issuingcountry) LIKE @filter
        ...
        public List<NotificationModel> GetNotifications(string filter)
        {
            ...
            {
                conn.Open();
                string query = @"SELECT n.NotificationId, n.userid, n.controllerid, c.fullname, n.message, n.link, n.created_at,
                ...
                if (!string.IsNullOrWhiteSpace(filter))
                {
                    query += @" AND (LOWER(n.message) LIKE @filter
                          ...
                          OR LOWER(c.fullname) LIKE @filter
                          OR LOWER(n.licensetype) LIKE @filter)"; // Changed : to @
                }
    ...
    ";
        ...
        private Employee MapDataRowToEmployee(DataRow row)
        {
            return new Employee
            {
                ...
                // تأكد من إضافة جميع الخصائص الأخرى هنا
            };
        }
